...
<SizePropsCompareList icon={'country-flag-gr'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-gr'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Greece'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-gr'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-gr'} ... /> // Uses the default ('Greece')
<CarmoreIcon icon={'country-flag-gr'} aria-label={'Flag'} ... />
```
