...
<SizePropsCompareList icon={'country-flag-no'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-no'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Norway'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-no'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-no'} ... /> // Uses the default ('Norway')
<CarmoreIcon icon={'country-flag-no'} aria-label={'Flag'} ... />
```
