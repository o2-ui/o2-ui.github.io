...
<SizePropsCompareList icon={'country-flag-pa'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-pa'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Panama'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-pa'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-pa'} ... /> // Uses the default ('Panama')
<CarmoreIcon icon={'country-flag-pa'} aria-label={'Flag'} ... />
```
