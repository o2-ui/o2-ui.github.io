...
<SizePropsCompareList icon={'country-flag-sk'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-sk'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Slovakia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-sk'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-sk'} ... /> // Uses the default ('Slovakia')
<CarmoreIcon icon={'country-flag-sk'} aria-label={'Flag'} ... />
```
