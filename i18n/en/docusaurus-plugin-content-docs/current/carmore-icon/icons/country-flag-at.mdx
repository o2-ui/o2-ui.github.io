...
<SizePropsCompareList icon={'country-flag-at'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-at'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Austria'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-at'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-at'} ... /> // Uses the default ('Austria')
<CarmoreIcon icon={'country-flag-at'} aria-label={'Flag'} ... />
```
