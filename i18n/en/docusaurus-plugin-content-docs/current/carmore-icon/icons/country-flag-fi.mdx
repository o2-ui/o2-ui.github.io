...
<SizePropsCompareList icon={'country-flag-fi'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-fi'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Finland'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-fi'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-fi'} ... /> // Uses the default ('Finland')
<CarmoreIcon icon={'country-flag-fi'} aria-label={'Flag'} ... />
```
