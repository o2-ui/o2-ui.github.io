...
<SizePropsCompareList icon={'country-flag-hu'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-hu'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Hungary'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-hu'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-hu'} ... /> // Uses the default ('Hungary')
<CarmoreIcon icon={'country-flag-hu'} aria-label={'Flag'} ... />
```
