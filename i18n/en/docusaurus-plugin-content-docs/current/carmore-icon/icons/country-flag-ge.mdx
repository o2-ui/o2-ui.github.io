...
<SizePropsCompareList icon={'country-flag-ge'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ge'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Georgia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ge'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ge'} ... /> // Uses the default ('Georgia')
<CarmoreIcon icon={'country-flag-ge'} aria-label={'Flag'} ... />
```
