...
<SizePropsCompareList icon={'country-flag-is'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-is'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Iceland'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-is'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-is'} ... /> // Uses the default ('Iceland')
<CarmoreIcon icon={'country-flag-is'} aria-label={'Flag'} ... />
```
