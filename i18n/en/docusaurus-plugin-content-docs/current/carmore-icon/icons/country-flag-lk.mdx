...
<SizePropsCompareList icon={'country-flag-lk'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-lk'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Sri Lanka'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-lk'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-lk'} ... /> // Uses the default ('Sri Lanka')
<CarmoreIcon icon={'country-flag-lk'} aria-label={'Flag'} ... />
```
