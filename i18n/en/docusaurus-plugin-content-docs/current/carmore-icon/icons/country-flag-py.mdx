...
<SizePropsCompareList icon={'country-flag-py'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-py'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Paraguay'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-py'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-py'} ... /> // Uses the default ('Paraguay')
<CarmoreIcon icon={'country-flag-py'} aria-label={'Flag'} ... />
```
