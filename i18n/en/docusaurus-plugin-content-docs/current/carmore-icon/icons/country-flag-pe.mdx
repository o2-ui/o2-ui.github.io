...
<SizePropsCompareList icon={'country-flag-pe'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-pe'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Peru'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-pe'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-pe'} ... /> // Uses the default ('Peru')
<CarmoreIcon icon={'country-flag-pe'} aria-label={'Flag'} ... />
```
