...
<SizePropsCompareList icon={'country-flag-za'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-za'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'South Africa'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-za'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-za'} ... /> // Uses the default ('South Africa')
<CarmoreIcon icon={'country-flag-za'} aria-label={'Flag'} ... />
```
