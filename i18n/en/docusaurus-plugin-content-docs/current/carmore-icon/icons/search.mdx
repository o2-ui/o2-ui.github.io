...
```tsx
<CarmoreIcon icon={'search'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Search'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'search'} ariaLabels={[undefined, 'Browse', 'Magnifier']} />

```tsx
<CarmoreIcon icon={'search'} ... /> // Uses the default ('Search')
