...
<SizePropsCompareList icon={'country-flag-al'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-al'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Albania'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-al'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-al'} ... /> // Uses the default ('Albania')
<CarmoreIcon icon={'country-flag-al'} aria-label={'Flag'} ... />
```
