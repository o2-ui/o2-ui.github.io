...
<SizePropsCompareList icon={'country-flag-la'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-la'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Laos'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-la'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-la'} ... /> // Uses the default ('Laos')
<CarmoreIcon icon={'country-flag-la'} aria-label={'Flag'} ... />
```
