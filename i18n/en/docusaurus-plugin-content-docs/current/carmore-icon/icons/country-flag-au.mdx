...
<SizePropsCompareList icon={'country-flag-au'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-au'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Australia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-au'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-au'} ... /> // Uses the default ('Australia')
<CarmoreIcon icon={'country-flag-au'} aria-label={'Flag'} ... />
```
