...
<SizePropsCompareList icon={'country-flag-il'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-il'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Israel'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-il'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-il'} ... /> // Uses the default ('Israel')
<CarmoreIcon icon={'country-flag-il'} aria-label={'Flag'} ... />
```
