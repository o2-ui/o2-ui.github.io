...
<SizePropsCompareList icon={'country-flag-fr'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-fr'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'France'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-fr'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-fr'} ... /> // Uses the default ('France')
<CarmoreIcon icon={'country-flag-fr'} aria-label={'Flag'} ... />
```
