...
<SizePropsCompareList icon={'country-flag-mt'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-mt'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Malta'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-mt'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-mt'} ... /> // Uses the default ('Malta')
<CarmoreIcon icon={'country-flag-mt'} aria-label={'Flag'} ... />
```
