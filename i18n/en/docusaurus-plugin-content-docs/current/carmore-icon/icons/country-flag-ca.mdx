...
<SizePropsCompareList icon={'country-flag-ca'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ca'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Canada'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ca'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ca'} ... /> // Uses the default ('Canada')
<CarmoreIcon icon={'country-flag-ca'} aria-label={'Flag'} ... />
```
