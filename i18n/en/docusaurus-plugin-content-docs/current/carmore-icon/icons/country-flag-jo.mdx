...
<SizePropsCompareList icon={'country-flag-jo'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-jo'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Jordan'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-jo'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-jo'} ... /> // Uses the default ('Jordan')
<CarmoreIcon icon={'country-flag-jo'} aria-label={'Flag'} ... />
```
