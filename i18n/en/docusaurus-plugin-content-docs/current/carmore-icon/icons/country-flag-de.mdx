...
<SizePropsCompareList icon={'country-flag-de'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-de'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Germany'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-de'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-de'} ... /> // Uses the default ('Germany')
<CarmoreIcon icon={'country-flag-de'} aria-label={'Flag'} ... />
```
