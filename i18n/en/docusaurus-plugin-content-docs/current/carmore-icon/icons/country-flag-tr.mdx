...
<SizePropsCompareList icon={'country-flag-tr'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-tr'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Turkey'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-tr'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-tr'} ... /> // Uses the default ('Turkey')
<CarmoreIcon icon={'country-flag-tr'} aria-label={'Flag'} ... />
```
