...
<SizePropsCompareList icon={'country-flag-hr'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-hr'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Croatia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-hr'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-hr'} ... /> // Uses the default ('Croatia')
<CarmoreIcon icon={'country-flag-hr'} aria-label={'Flag'} ... />
```
