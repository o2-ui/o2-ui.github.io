...
<SizePropsCompareList icon={'country-flag-me'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-me'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Montenegro'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-me'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-me'} ... /> // Uses the default ('Montenegro')
<CarmoreIcon icon={'country-flag-me'} aria-label={'Flag'} ... />
```
