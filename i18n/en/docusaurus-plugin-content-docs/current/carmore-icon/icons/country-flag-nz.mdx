...
<SizePropsCompareList icon={'country-flag-nz'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-nz'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'New Zealand'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-nz'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-nz'} ... /> // Uses the default ('New Zealand')
<CarmoreIcon icon={'country-flag-nz'} aria-label={'Flag'} ... />
```
