...
<SizePropsCompareList icon={'country-flag-lu'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-lu'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Luxembourg'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-lu'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-lu'} ... /> // Uses the default ('Luxembourg')
<CarmoreIcon icon={'country-flag-lu'} aria-label={'Flag'} ... />
```
