...
<SizePropsCompareList icon={'country-flag-it'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-it'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Italy'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-it'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-it'} ... /> // Uses the default ('Italy')
<CarmoreIcon icon={'country-flag-it'} aria-label={'Flag'} ... />
```
