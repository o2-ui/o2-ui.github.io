...
<SizePropsCompareList icon={'country-flag-eg'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-eg'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Egypt'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-eg'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-eg'} ... /> // Uses the default ('Egypt')
<CarmoreIcon icon={'country-flag-eg'} aria-label={'Flag'} ... />
```
