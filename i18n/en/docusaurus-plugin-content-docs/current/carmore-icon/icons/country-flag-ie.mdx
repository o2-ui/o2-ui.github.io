...
<SizePropsCompareList icon={'country-flag-ie'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ie'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Ireland'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ie'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ie'} ... /> // Uses the default ('Ireland')
<CarmoreIcon icon={'country-flag-ie'} aria-label={'Flag'} ... />
```
