...
<SizePropsCompareList icon={'country-flag-gb'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-gb'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'United Kingdom'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-gb'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-gb'} ... /> // Uses the default ('United Kingdom')
<CarmoreIcon icon={'country-flag-gb'} aria-label={'Flag'} ... />
```
