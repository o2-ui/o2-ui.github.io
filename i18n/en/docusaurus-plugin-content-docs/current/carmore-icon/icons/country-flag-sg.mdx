...
<SizePropsCompareList icon={'country-flag-sg'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-sg'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Singapore'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-sg'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-sg'} ... /> // Uses the default ('Singapore')
<CarmoreIcon icon={'country-flag-sg'} aria-label={'Flag'} ... />
```
