...
<SizePropsCompareList icon={'country-flag-cz'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-cz'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Czechia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-cz'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-cz'} ... /> // Uses the default ('Czechia')
<CarmoreIcon icon={'country-flag-cz'} aria-label={'Flag'} ... />
```
