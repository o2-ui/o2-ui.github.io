...
<SizePropsCompareList icon={'country-flag-ch'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ch'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Switzerland'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ch'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ch'} ... /> // Uses the default ('Switzerland')
<CarmoreIcon icon={'country-flag-ch'} aria-label={'Flag'} ... />
```
