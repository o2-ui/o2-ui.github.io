...
<SizePropsCompareList icon={'country-flag-tw'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-tw'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Taiwan'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-tw'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-tw'} ... /> // Uses the default ('Taiwan')
<CarmoreIcon icon={'country-flag-tw'} aria-label={'Flag'} ... />
```
