...
<SizePropsCompareList icon={'country-flag-si'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-si'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Slovenia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-si'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-si'} ... /> // Uses the default ('Slovenia')
<CarmoreIcon icon={'country-flag-si'} aria-label={'Flag'} ... />
```
