...
<SizePropsCompareList icon={'country-flag-ec'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ec'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Ecuador'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ec'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ec'} ... /> // Uses the default ('Ecuador')
<CarmoreIcon icon={'country-flag-ec'} aria-label={'Flag'} ... />
```
