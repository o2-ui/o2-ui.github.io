...
<ColorPropsCompareList icon={'user'} colors={['red', 'orange', 'green', 'blue', 'purple']} />
...
- The default `aria-label` is 'User'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'user'} ariaLabels={[undefined, 'Profile', 'User']} />

```tsx
<CarmoreIcon icon={'user'} ... /> // Uses the default ('User')
<CarmoreIcon icon={'user'} aria-label={'Profile'} ... />
<CarmoreIcon icon={'user'} aria-label={'User'} ... />
```
