...
<SizePropsCompareList icon={'country-flag-kw'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-kw'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Kuwait'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-kw'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-kw'} ... /> // Uses the default ('Kuwait')
<CarmoreIcon icon={'country-flag-kw'} aria-label={'Flag'} ... />
```
