...
<SizePropsCompareList icon={'country-flag-ee'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ee'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Estonia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ee'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ee'} ... /> // Uses the default ('Estonia')
<CarmoreIcon icon={'country-flag-ee'} aria-label={'Flag'} ... />
```
