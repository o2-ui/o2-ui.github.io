...
<SizePropsCompareList icon={'country-flag-sa'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-sa'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Saudi Arabia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-sa'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-sa'} ... /> // Uses the default ('Saudi Arabia')
<CarmoreIcon icon={'country-flag-sa'} aria-label={'Flag'} ... />
```
