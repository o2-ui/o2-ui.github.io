...
<SizePropsCompareList icon={'country-flag-ar'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ar'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Argentina'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ar'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ar'} ... /> // Uses the default ('Argentina')
<CarmoreIcon icon={'country-flag-ar'} aria-label={'Flag'} ... />
```
