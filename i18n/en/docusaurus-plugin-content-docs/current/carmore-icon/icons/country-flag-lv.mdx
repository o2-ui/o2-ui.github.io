...
<SizePropsCompareList icon={'country-flag-lv'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-lv'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Latvia'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-lv'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-lv'} ... /> // Uses the default ('Latvia')
<CarmoreIcon icon={'country-flag-lv'} aria-label={'Flag'} ... />
```
