...
<SizePropsCompareList icon={'share'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'share'} ... /> // Uses the default ('1.5rem')
...
<ColorPropsCompareList icon={'share'} colors={['red', 'orange', 'green', 'blue', 'purple']} />
...
- The default `aria-label` is 'Share'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'share'} ariaLabels={[undefined, 'Upload', 'Upload']} />

```tsx
<CarmoreIcon icon={'share'} ... /> // Uses the default ('Share')
<CarmoreIcon icon={'share'} aria-label={'Upload'} ... />
<CarmoreIcon icon={'share'} aria-label={'Upload'} ... />
```
