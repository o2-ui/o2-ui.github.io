...
<SizePropsCompareList icon={'country-flag-es'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-es'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Spain'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-es'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-es'} ... /> // Uses the default ('Spain')
<CarmoreIcon icon={'country-flag-es'} aria-label={'Flag'} ... />
```
