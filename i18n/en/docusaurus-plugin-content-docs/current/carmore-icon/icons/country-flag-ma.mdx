...
<SizePropsCompareList icon={'country-flag-ma'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-ma'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Morocco'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-ma'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-ma'} ... /> // Uses the default ('Morocco')
