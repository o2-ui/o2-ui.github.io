...
<SizePropsCompareList icon={'country-flag-cr'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-cr'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Costa Rica'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-cr'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-cr'} ... /> // Uses the default ('Costa Rica')
