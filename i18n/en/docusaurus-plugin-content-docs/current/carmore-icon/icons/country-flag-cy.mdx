...
<SizePropsCompareList icon={'country-flag-cy'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-cy'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Cyprus'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-cy'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-cy'} ... /> // Uses the default ('Cyprus')
<CarmoreIcon icon={'country-flag-cy'} aria-label={'Flag'} ... />
```
