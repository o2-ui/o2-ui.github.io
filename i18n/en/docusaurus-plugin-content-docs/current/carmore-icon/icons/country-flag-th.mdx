...
<SizePropsCompareList icon={'country-flag-th'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-th'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Thailand'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-th'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-th'} ... /> // Uses the default ('Thailand')
<CarmoreIcon icon={'country-flag-th'} aria-label={'Flag'} ... />
```
