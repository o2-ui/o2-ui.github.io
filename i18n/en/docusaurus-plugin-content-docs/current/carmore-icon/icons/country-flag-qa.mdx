...
<SizePropsCompareList icon={'country-flag-qa'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'country-flag-qa'} ... /> // Uses the default ('1.5rem')
...
- The default `aria-label` is 'Qatar'.
- Base accessibility defaults are provided, and you can override them when needed.
- _(Hover the icon to check the tooltip.)_

<A11yPropsCompareList icon={'country-flag-qa'} ariaLabels={[undefined, 'Flag']} />

```tsx
<CarmoreIcon icon={'country-flag-qa'} ... /> // Uses the default ('Qatar')
<CarmoreIcon icon={'country-flag-qa'} aria-label={'Flag'} ... />
```
