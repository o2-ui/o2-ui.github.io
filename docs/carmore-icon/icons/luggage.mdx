...
<SizePropsCompareList icon={'luggage'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'luggage'} ... /> // 기본값('1.5rem') 적용
...
<ColorPropsCompareList icon={'luggage'} colors={['red', 'orange', 'green', 'blue', 'purple']} />
...
- `role` 의 기본값은 'img' 입니다.
- `aria-label` 의 기본값은 '수화물' 입니다.
- 기본 접근성 속성이 적용되어 있지만, 필요에 따라 커스터마이징할 수 있습니다.
- _(아이콘에 마우스를 올려보세요.)_

<A11yPropsCompareList icon={'luggage'} ariaLabels={[undefined, '기내 수하물', '위탁 수하물']} />

```tsx
<CarmoreIcon icon={'luggage'} ... /> // 기본값('수화물') 적용
<CarmoreIcon icon={'luggage'} aria-label={'기내 수하물'} ... />
<CarmoreIcon icon={'luggage'} aria-label={'위탁 수하물'} ... />
```
