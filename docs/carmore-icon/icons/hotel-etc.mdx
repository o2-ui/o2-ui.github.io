...
<br />
<DeprecatedBoard title={'사용 불가 알림'} description={'이 컴포넌트는 더 이상 사용할 수 없습니다. 아래 컴포넌트를 대신 사용해주세요.'} altIcon={'etc'} />
...
```tsx
<CarmoreIcon icon={'hotel-etc'} ... /> // 기본값('1.5rem') 적용
...
<A11yPropsCompareList icon={'hotel-etc'} ariaLabels={[undefined, '내용확인', '체크마크']} />

```tsx
<CarmoreIcon icon={'hotel-etc'} ... /> // 기본값('확인사항') 적용
<CarmoreIcon icon={'hotel-etc'} aria-label={'내용확인'} ... />
<CarmoreIcon icon={'hotel-etc'} aria-label={'체크마크'} ... />
```
