...
<SizeCompareList icon={'search'} sizes={['1.5rem', 30, '48px', '5rem']} />

```tsx
<CarmoreIcon icon={'search'} ... /> // 기본값('1.5rem') 적용
...
<ColorCompareList icon={'search'} colors={['red', 'orange', 'green', 'blue', 'purple']} />
...
- `role` 의 기본값은 'img' 입니다.
- `aria-label` 의 기본값은 '검색하기' 입니다.
- 기본 접근성 속성이 적용되어 있지만, 필요에 따라 커스터마이징할 수 있습니다.
- _(아이콘에 마우스를 올려보세요.)_

<A11yCompareList icon={'search'} ariaLabels={[undefined, '찾아보기', '돋보기']} />

```tsx
<CarmoreIcon icon={'search'} ... /> // 기본값('검색하기') 적용
<CarmoreIcon icon={'search'} aria-label={'찾아보기'} ... />
<CarmoreIcon icon={'search'} aria-label={'돋보기'} ... />
```
