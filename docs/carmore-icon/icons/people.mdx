...
```tsx
<CarmoreIcon icon={'people'} ... /> // 기본값('1.5rem') 적용
...
- `role` 의 기본값은 'img' 입니다.
- `aria-label` 의 기본값은 '사람' 입니다.
- 기본 접근성 속성이 적용되어 있지만, 필요에 따라 커스터마이징할 수 있습니다.
- _(아이콘에 마우스를 올려보세요.)_

<A11yPropsCompareList icon={'people'} ariaLabels={[undefined, '여행객', '사용자']} />

```tsx
<CarmoreIcon icon={'people'} ... /> // 기본값('사람') 적용
<CarmoreIcon icon={'people'} aria-label={'여행객'} ... />
<CarmoreIcon icon={'people'} aria-label={'사용자'} ... />
```
