...
```tsx
<CarmoreIcon icon={'business'} ... /> // 기본값('1.5rem') 적용
...
<A11yPropsCompareList icon={'business'} ariaLabels={[undefined, '비지니스', '영업']} />

```tsx
<CarmoreIcon icon={'business'} ... /> // 기본값('서류가방') 적용
<CarmoreIcon icon={'business'} aria-label={'비지니스'} ... />
<CarmoreIcon icon={'business'} aria-label={'영업'} ... />
```
...
<UpdateLog
  logList={[
    {
      version: '1.4.0',
      content: '컴포넌트명 변경: HotelBusiness -> Business',
    },
  ]}
/>
