...
```tsx
<CarmoreIcon icon={'vehicle-year'} ... /> // 기본값('1.5rem') 적용
...
- `role` 의 기본값은 'img' 입니다.
- `aria-label` 의 기본값은 '연식' 입니다.
- 기본 접근성 속성이 적용되어 있지만, 필요에 따라 커스터마이징할 수 있습니다.
- _(아이콘에 마우스를 올려보세요.)_

<A11yPropsCompareList icon={'vehicle-year'} ariaLabels={[undefined, '제조 연도', '연형']} />

```tsx
<CarmoreIcon icon={'vehicle-year'} ... /> // 기본값('연식') 적용
<CarmoreIcon icon={'vehicle-year'} aria-label={'제조 연도'} ... />
<CarmoreIcon icon={'vehicle-year'} aria-label={'연형'} ... />
```
