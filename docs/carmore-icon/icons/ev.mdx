...
```tsx
<CarmoreIcon icon={'ev'} ... /> // 기본값('1.5rem') 적용
...
- `role` 의 기본값은 'img' 입니다.
- `aria-label` 의 기본값은 '전기차' 입니다.
- 기본 접근성 속성이 적용되어 있지만, 필요에 따라 커스터마이징할 수 있습니다.
- _(아이콘에 마우스를 올려보세요.)_

<A11yPropsCompareList icon={'ev'} ariaLabels={[undefined, '전자제품', '전압']} />

```tsx
<CarmoreIcon icon={'ev'} ... /> // 기본값('전기차') 적용
<CarmoreIcon icon={'ev'} aria-label={'전자제품'} ... />
<CarmoreIcon icon={'ev'} aria-label={'전압'} ... />
```
